...
* **`[SerializedField]` 사용은 선택 사항**입니다. 필드를 직렬화해야 할 명확한 이유가 있을 때만 사용합니다.
    * 코드 리뷰 시 `[SerializedField]` 속성의 존재 여부를 **강요하거나 수정 요청하지 않습니다.**
    * 접근 제어자(`private`, `protected`, `public`)와 직렬화 속성은 **서로 독립적으로 판단**합니다.
